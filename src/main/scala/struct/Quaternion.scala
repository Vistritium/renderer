...
object Quaternion {
  ...
  def rotationYawPitchRoll(yaw: Float, pitch: Float, roll: Float) = {
    ...
    var sinRoll = FastMath.sin(halfRoll)
    var cosRoll = FastMath.cos(halfRoll)
    var sinPitch = FastMath.sin(halfPitch)
    var cosPitch = FastMath.cos(halfPitch)
    var sinYaw = FastMath.sin(halfYaw)
    var cosYaw = FastMath.cos(halfYaw)
...
class Quaternion(ar: Float, ax: Float, ay: Float, az: Float) {
  ...
  //angle in radian
  def this(angle: Float, axis: Vector3) = {
  ...
  def setRotation(angle: Float, axis: Vector3): Unit = {
    val tmp: Vector3 = axis.normalised
  ...
  def rotate(v: Vector3): Vector3 = {
    ...
    val uv: Vector3 = qvec cross v
    val uuv: Vector3 = qvec cross uv
    uv *= (2.0f * r)
    uuv *= 2.0f
    return v + uv + uuv
  }
  ...
  def getMatrix: Matrix4 = {
    ...
    result(1) = 2.0f * (x * y + z * r)
    result(2) = 2.0f * (x * z - y * r)
    result(3) = 0.0f

    result(4) = 2.0f * (x * y - z * r)
    result(5) = 1.0f - 2.0f * (x * x + z * z)
    result(6) = 2.0f * (z * y + x * r)
    result(7) = 0.0f

    result(8) = 2.0f * (x * z + y * r)
    result(9) = 2.0f * (y * z - x * r)
